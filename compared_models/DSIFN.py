...
class DSIFN(nn.Module):
    ...
    def forward(self, t1_input, t2_input=None, is_train=False):
        if t2_input is None:
            t2_input = t1_input
        ...
        if self.do_sigmoid:
            branch_1_out = self.sigmoid(branch_1_out)
        ...
        if self.do_sigmoid:
            branch_2_out = self.sigmoid(branch_2_out)
        ...
        if self.do_sigmoid:
            branch_3_out=self.sigmoid(branch_3_out)
        ...
        if self.do_sigmoid:
            branch_4_out=self.sigmoid(branch_4_out)
        ...
        if self.do_sigmoid:
            branch_5_out = self.sigmoid(branch_5_out)
