...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256,
                 pretrained=f'{base_dir}/pretrained/pretrained_changeformer.pt'):
        ...
        save_model = torch.load(pretrained)
        model_dict = self.state_dict()
        state_dict = {k: v for k, v in save_model.items() if k in model_dict.keys()}
        model_dict.update(state_dict)
        self.load_state_dict(model_dict)

    def forward(self, x1, x2, is_train=False):
        h, w = x1.shape[2:]
        ...
        # exit()
        cp = [resize(item, size=[h, w], mode='bilinear') for item in cp]
