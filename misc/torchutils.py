...
def pred_large(net, A, input_size=256, stride=256,
    ...
    new_A[:, :, :h, :w] = A
    new_pred = torch.zeros([n, 1, new_h, new_w], dtype=torch.float32)
    del A
    #
    for i in range(0, new_h - input_size + 1, stride):
        for j in range(0, new_w - input_size + 1, stride):
            ...
            A = patch_A.to(device)
            with torch.no_grad():
                patch_pred = net.forward(A)
                new_pred[:, :, top:bottom, left:right] = patch_pred.detach().cpu()
    pred = new_pred[:, :, :h, :w]
    return pred


def pred_large_cd(net, A, B, input_size=256, stride=256,
                  device=torch.device('cuda:0')):
    """
    输入前后时相的大图，获得预测结果
    假定预测结果中心部分为准确，边缘padding = (input_size-stride)/2
    ...
    for i in range(0, new_h - input_size + 1, stride):
        for j in range(0, new_w - input_size + 1, stride):
            ...
            B = patch_B.to(device)
            with torch.no_grad():
                patch_pred = net.forward(A, B)
                new_pred[:, :, top:bottom, left:right] = patch_pred.detach().cpu()
    pred = new_pred[:, :, :h, :w]
    return pred


def tensor_mask_colorize(label_mask):
    """
    :param label_mask: mask (np.ndarray): (M, N), uint8
    :return: color label: (M, N, 3), uint8
    """
    assert isinstance(label_mask, torch.Tensor)
    assert label_mask.ndim == 4
    #TODO
...
def seed_torch(seed=2019):
    ...
    #  我发现，F.cross_entropy 居然没有确定性的结果。
...
def class2one_hot(seg: Tensor, C: int) -> Tensor:
    ...
    if len(seg.shape) == 4:  # n, 1, w, h, used by the dataloader
...
def save_visuals(visuals, img_dir, name, save_one=True, iter='0', if_normalize=True):
    """
    """
    ...
    for label, image in visuals.items():
        ...
        for j in range(N):
            ...
            name_ = name_.split(".")[0]
            # print(name_)
            image_numpy = tensor2np(image[j], if_normalize=if_normalize).astype(np.uint8)
            # print(image_numpy)
...
def save_feature_maps(save_folder, tensor_data, name_prefix,
                      name_suffix='', colormode='jet'):
    """
    colormode: twilight | jet | rainbow
    """
    np_data = tensor2np(tensor_data)
    if np_data.ndim == 2:
        color_data = apply_colormap(np_data, colormode=colormode)
        save_image(color_data, os.path.join(save_folder, name_prefix+name_suffix+'.jpg'))
    elif np_data.ndim == 3:
        n_channel = np_data.shape[-1]
        for i in range(n_channel):
            feature_data = np_data[:, :, i]
            color_data = apply_colormap(feature_data, colormode=colormode)
            save_image(color_data, os.path.join(save_folder,
                                                name_prefix +'_'+ str(i) +
                                                name_suffix + '.jpg'))
    else:
        raise Exception
...
def mask_colorize(mask, label_colours):
    """
    按照指定索引给mask上色
    :param mask:
    :param label_colours:
    ...
    assert mask.ndim == 3
    b, h, w = mask.shape
    output = torch.zeros([b, h, w, 3], dtype=torch.uint8)
    max_num = mask.max().item()
    max_num = int(max_num)
    # 从0-max_num染色
    for j in range(b):
        for i in range(max_num+1):
            output[j][mask[j]==i] = label_colours[i]
